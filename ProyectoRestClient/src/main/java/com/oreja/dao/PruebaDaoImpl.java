...
public class PruebaDaoImpl implements IPruebaDao {
    ...
    private static List<Prueba> pruebas;
    ...
    public List<Prueba> getPruebasByMaterial (int codigo) {
	    List<Prueba> pMat = pruebas.stream()
		.filter(p -> p.getMaterial().getCodigo() == codigo)
		.collect(Collectors.toList());
	    if(pMat.size() < 1 || pMat == null) {
	...
	getPruebasByMaterial(codigo).stream()
		.forEach(p -> deletePrueba(p));
    }
    
    public void saveChanges() {
	try { 
	    Files.write(file.toPath(), jsonUtil.serialize(pruebas).getBytes(), StandardOpenOption.CREATE);
